...
#urban vs not
b_urb<-ggplot(HE_abncountB, aes(x=Urban, y=prop.abn, fill=Treatment)) +
  geom_boxplot()+ theme_box()+
  #geom_line(aes(color= MatePair, group=MatePair))+
  labs(title = 'Blastula', x = 'Population', y = 'Proportion Abnormalities')+
  scale_fill_manual(values = c("NP" = "#C77DFF", "C" = "#C8E9A0"))

ggsave("blastula_urban.png",b_urb, width=15, height=10, units = "cm") 

#test for abn variation with treatment, blocking by population
bstats<-lmer(prop.abn~Treatment*Population+ (1|Experiment), data=HE_abncountB) 
anova(bstats) 
...
leveneTest(ratio~Treatment*Urban, data=HE_abncountB) #no difference in homogeneity
...
ggplot(HE_abntypeB_full, aes(fill=abnormality_type, y=totalprop_abn, x=Treatment)) +
  ...
  facet_wrap("Urban")+
  theme_classic()+
  labs(title = 'Blastula Abnormalities Observed', x = 'Treatment', y = 'Number Blastula Abnormalities Observed Across Treatments')+  
...
abnGg<-
  ...
  geom_boxplot()+ theme_box()+
  labs(title = 'Gastrula ', x = 'Populations',
       y = 'Proportion of Abnormalities')+
    scale_fill_manual(values = c("NP" = "#C77DFF", "C" = "#C8E9A0"))
  
ggsave("gastrula_urban.png",abnGg, width=20, height=11, units = "cm")   
...
#lets test for the amount of variance within vs between populations for survival relative to control
#Levene's test, more flexible if data isn't normally distributed
leveneTest(ratio~Treatment*Urban, data=HE_abncountG) #no difference in homogeneity
...
ggplot(HE_abncountP, aes(x=Urban, y=ratio, fill=Treatment)) +
 ...
 # facet_wrap(~Population)+
#  geom_line(aes(color= MatePair, group=MatePair))+
  labs(title = 'Pluteus', x = 'Populations',
       y = 'Proportion of Abnormalities')+
  scale_fill_manual(values = c("NP" = "#C77DFF", "C" = "#C8E9A0"))

ggsave("pluteus_urban.png",p_urb, width=20, height=11, units = "cm")
...
#lets test for the amount of variance within vs between populations for survival relative to control
#Levene's test, more flexible if data isn't normally distributed
leveneTest(ratio~Treatment*Population, data=HE_abncountP) #no difference in homogeneity
...
ggplot(HE_abntypeP_full, aes(fill=abnormality_type, y=totalprop_abn, x=Treatment)) +
  ...
  facet_wrap("Urban")+
  theme_classic()+
  labs(title = 'Pluteus Abnormalities Observed', x = 'Treatment', y = 'Abnormalities Observed Across Treatments')+  
  scale_fill_brewer(palette="Set3")
...
ggplot(HE_abntypeP_nosmall, aes(fill=abnormality_type, y=totalprop_abn, x=Treatment)) +
  geom_bar(position="stack", stat="identity")+ 
  facet_wrap("Population")+
  theme_classic()+
  labs(title = 'Pluteus Abnormalities Observed', x = 'Treatment', y = 'Abnormalities Observed Across Treatments')+  
...
ggplot(HE_abntypeP_ratios, aes(x=Urban, y=totalprop_abn, fill=Treatment)) +
  geom_boxplot()+ theme_box()+
  labs(title = 'Proportion Abnormal Pluteus w/o small pluts', x = 'Populations',
       y = 'Proportion of Abnormal PLuteus')+
  scale_fill_manual(values = c("NP" = "#C77DFF", "C" = "#C8E9A0"))

#stats
#test for abn variation with treatment, blocking by population
pstatsW<-lmer(totalprop_abn~Treatment*Population+ (1|Experiment), data=HE_abntypeP_ratios) 
anova(pstatsW) 
